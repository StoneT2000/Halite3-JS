...
game.initialize().then(async () => {
  ...
  await game.ready('Current Bot: SM-Bot3 Dec-24');
  ...
  while (true) {
    ...
    for (let i = 0; i < gameMap.width; i++) {
      for (let j = 0; j < gameMap.height; j++) {
        ...
        if (buildableLocation && distanceToNearestDropoff >= 2 * 6){
    ...
    if (numDropoffs + shipsDesignatedToBuild < maxDropoffs && buildDropoffs === true && game.turnNumber <= 0.85 * hlt.constants.MAX_TURNS) {
      ...
      let possibleShipPositions = search.circle(gameMap, nextDropoffSpot.position, 6);
